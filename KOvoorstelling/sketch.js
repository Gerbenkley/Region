...
let text2 = 'Met de muis beweegt u tussen locaties. Klik en sleep voor meer interactie!'
let text3 = 'Om te beginnen, klik ergens.'
let text4 = 'Mede mogelijk gemaakt door: Gerben van der Kleij, Chris Bol, Victor Dissel'
...
function drawTree() {
  ...
  translate(windowWidth/4 + mouseX/20, windowHeight-125);
...
function mouseDragged() {
    ...
    if (mouseX < windowWidth/3) {
      ...
      let ijsspeed = map(mouseY, 0.1, height, 2, 0);
          ijsspeed = constrain(ijsspeed, 0.05, 4);
              ijs.rate(ijsspeed);
        ...
        let ijsmuisy = map(mouseY, 1, windowHeight, 2, 25);
        ...
        let vogspeed = map(mouseY, 0.1, height, 0.75, 1.25);
            ...
            if (flappers.isPlaying() == false) {
              flappers.setVolume(0);
              flappers.stop();
              flappers.play();
              flappers.setVolume(0.2, 1);
            }
